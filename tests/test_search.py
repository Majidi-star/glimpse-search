...
class TestSearch:
    def setup_method(self):
        ...
        for suffix in ["", "-wal", "-shm"]:
            p = (
                ...
                if suffix == ""
                else self.db_path.parent / (self.db_path.name + suffix)
            )
            if p.exists():
                p.unlink()
    ...
    def test_vector_search(self):
        ...
        with connect(self.db_path) as con:
            ...
            hits = hybrid_search(
                con, query_embedding=serialize_embedding(query_vec), query_text="", top_k=10
            )
    ...
    def test_keyword_search(self):
        ...
        with connect(self.db_path) as con:
            hits = hybrid_search(
                con,
                query_embedding=serialize_embedding(self.embedder.embed_texts(["x"])[0]),
                query_text="hello",
                top_k=10,
            )
    ...
    def test_hybrid_rerank(self):
        ...
        self._add_chunks(
            file_id, ["exact match phrase", "similar concept here", "unrelated content"]
        )
        ...
        with connect(self.db_path) as con:
            ...
            hits = hybrid_search(
                con,
                query_embedding=serialize_embedding(query_vec),
